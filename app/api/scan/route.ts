...
FAIL reason: "No payment logos or security badges (Visa, Mastercard, SSL, PayPal) were found on the product page. Add payment method logos near the Add to Cart button to build trust."
            ...
            specialInstructions = `
You are an AI rule validation engine.
Your task is to determine whether this webpage contains a valid Product Comparison Section.
The comparison may appear in two formats:
1. DOM-based comparison (HTML tables, grids, structured layouts)
2. Image-based comparison (comparison tables or charts embedded in images)
You must analyze BOTH the page content/DOM AND the screenshot image provided.

STEP 1: DOM ANALYSIS
Scan the DOM/HTML content for structured comparison patterns.
Look for:
- HTML tables (table, thead, tbody, tr, th, td elements)
- Grid or card layouts comparing multiple products
- Sections with headings such as: "Product Comparison", "Compare Products", "Comparison Table", "Feature Comparison", "Side-by-side comparison", "vs", "More Powerful Than", "How We Stack Up", "Why Choose Us"

Validation rules for DOM comparison — valid only if ALL of:
1. At least 2 different products/alternatives are being compared (competitor products, a category like "Coffee", product tiers, etc.)
2. At least 4 attributes/features/rows are listed
3. Attributes are displayed in a structured format (table rows, grid columns, checkmark/X rows)

Valid attributes include: Price, Size, Material, Weight, Battery Life, Performance, Durability, Warranty, Capacity, Speed, Dimensions, Zero crashes, Plant-based, Functional Ingredients, Adaptogens, etc.

Reject DOM sections if: only one product, fewer than 4 attributes, marketing text without structure, or feature list of a single product.

STEP 2: IMAGE ANALYSIS
If no valid DOM comparison is detected, analyze the screenshot image carefully.
Detect whether the image shows a product comparison chart or table.

Indicators of image-based comparison:
- Multiple product names or categories in columns (e.g. "Rainbow Dust" vs "Coffee")
- Feature rows or attribute rows aligned across columns
- Comparison grids with checkmarks (✓) and X marks (✗)
- Visual spec tables or comparison charts

Validation rules for image comparison — valid only if ALL of:
1. It compares at least 2 products or categories
2. It contains at least 4 attribute or feature rows
3. The layout clearly represents a comparison structure (table, chart, or grid with aligned columns)

Reject images if: only product photos, lifestyle/marketing images, infographics without side-by-side product comparison, images showing only one product.

FINAL DECISION:
PASS if: A valid comparison is found in the DOM OR a valid comparison is found in the screenshot image.
FAIL if: No valid comparison is found in DOM AND no valid comparison is found in the image.

EXAMPLES:

✅ PASS — Image-based: Screenshot shows "5x More Powerful Than Coffee" section with a 2-column checkmark/X grid. Columns: Rainbow Dust (✓) vs Coffee (✗). Rows: Zero crashes/jitters, Plant-based & cruelty free, Functional Ingredients, Adaptogens for stress support, Variety of flavours, All-in-one daily blend (6 rows).
Output: { "passed": true, "reason": "A product comparison section is visible in the page image comparing Rainbow Dust against Coffee across 6 meaningful attributes (Zero crashes, Plant-based, Functional Ingredients, Adaptogens, Variety of flavours, All-in-one blend) in a side-by-side checkmark/X grid format." }

✅ PASS — DOM-based: Table found with 3 products compared across 6 attributes (Price, RAM, Battery, Performance, Warranty, Features) in a structured HTML table.
Output: { "passed": true, "reason": "Product comparison table found in DOM comparing 3 products across 6 attributes in a side-by-side table format." }

❌ FAIL — Insufficient attributes: A comparison section exists but only 2 attributes are compared.
Output: { "passed": false, "reason": "A comparison section exists but only compares 2 attributes. At least 4 meaningful attribute rows are required." }

❌ FAIL — No comparison found: No comparison section in DOM and no comparison visible in screenshot.
Output: { "passed": false, "reason": "No product comparison section found on the page. The rule requires a comparison section (product vs competitors, product vs a category, or product tiers) with at least 4 attributes in a visual table or checkmark/X grid format." }

CRITICAL INSTRUCTIONS:
1. ALWAYS check the screenshot image — comparison sections are often rendered as images, not HTML text
2. A comparison against a generic category (e.g. "vs Coffee") is VALID — no named competitor required
3. Each checkmark/X row counts as one attribute
4. Side-by-side checkmark/X grid IS a valid visual format
5. PASS if either DOM or image contains a valid comparison
6. State clearly whether the comparison was found in DOM, in the image, or both
              `
...
FAIL only if the screenshot does not show it AND FREE_SHIPPING_DOM_FOUND=false.
          ...
          const ruleSpecificPrefix = `${customerPhotoPrefix}${videoTestimonialPrefix}${productTabsPrefix}${trustBadgesPrefix}${benefitsNearTitlePrefix}${thumbnailsPrefix}${beforeAfterPrefix}${freeShippingThresholdPrefix}${comparisonPrefix}`
          ...
          if (isRatingRule) {
            ...
            // Product comparison rule must mention comparison-related terms
            const hasComparisonMention = reasonLower.includes('comparison') ||
              ...
              reasonLower.includes('checkmark') ||
              reasonLower.includes('check mark') ||
              reasonLower.includes('grid') ||
              reasonLower.includes('feature') ||
              reasonLower.includes('coffee') ||
              reasonLower.includes('side-by-side') ||
              reasonLower.includes('row')
            ...
            // Relaxed checks — only log warnings, never block a PASS
            const mentionsAttributes = reasonLower.includes('attribute') ||
              reasonLower.includes('feature') ||
              reasonLower.includes('row') ||
              reasonLower.includes('4') || reasonLower.includes('5') || reasonLower.includes('6') ||
              reasonLower.includes('ingredient') ||
              reasonLower.includes('benefit')
            const mentionsVisualFormat = reasonLower.includes('table') ||
              reasonLower.includes('format') ||
              reasonLower.includes('side-by-side') ||
              reasonLower.includes('checkmark') ||
              reasonLower.includes('grid') ||
              reasonLower.includes('✓') || reasonLower.includes('✗')

            if (analysis.passed && (!mentionsAttributes || !mentionsVisualFormat)) {
              console.warn(`Warning: Product comparison rule passed but missing some expected mentions. Attributes: ${mentionsAttributes}, Visual Format: ${mentionsVisualFormat}`)
            }
