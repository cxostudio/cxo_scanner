    ...
    try {
      ...
      if (captureScreenshot && !earlyScreenshot) {
        try {
          ...
          // Use Promise.race with timeout to avoid hanging on Vercel
          const screenshotPromise = page.screenshot({
            type: 'jpeg',
            fullPage: false, // Start with viewport only (faster, more reliable on Vercel)
            encoding: 'base64',
            quality: 70,
          }) as Promise<string>
          ...
          try {
            const earlyScreenshotBuffer = await Promise.race([screenshotPromise, timeoutPromise])
            earlyScreenshot = `data:image/jpeg;base64,${earlyScreenshotBuffer}`
            console.log(`Early screenshot captured successfully (viewport, length: ${earlyScreenshot.length} chars)`)

            // Try to get full page screenshot in background (non-blocking)
            page.screenshot({
              type: 'jpeg',
              fullPage: true,
              encoding: 'base64',
              quality: 75,
            }).then((fullPageBuffer: string) => {
              const fullPageScreenshot = `data:image/jpeg;base64,${fullPageBuffer}`
              earlyScreenshot = fullPageScreenshot // Update to full page if successful
              console.log(`Full page early screenshot captured (length: ${fullPageScreenshot.length} chars)`)
            }).catch((err) => {
              console.log('Full page early screenshot failed, keeping viewport version:', err)
            })
          } catch (screenshotError) {
            console.warn('Early screenshot capture failed:', screenshotError)
            // Continue without early screenshot, will try again later
          }
        } catch (earlyScreenshotError) {
          console.error('Failed to capture early screenshot completely:', earlyScreenshotError)
          // Continue without early screenshot, will try again later
        }
      }
      ...
      if (captureScreenshot) {
        if (earlyScreenshot) {
          ...
          console.log(`Using early screenshot (length: ${earlyScreenshot.length} chars)`)
        } else {
          // Try to capture final screenshot (if time permits)
          console.log('No early screenshot available, taking final screenshot...')
          try {
            ...
            console.log(`Final screenshot captured in JPEG format (length: ${screenshotDataUrl.length} chars)`)
          } catch (screenshotError) {
            console.warn('Failed to capture final screenshot:', screenshotError)
            // Try one more time with lower quality if first attempt failed
            try {
              console.log('Retrying screenshot with lower quality (viewport only)...')
              ...
              screenshotDataUrl = `data:image/jpeg;base64,${retryScreenshot}`
              console.log(`Retry screenshot captured (length: ${screenshotDataUrl.length} chars)`)
            } catch (retryError) {
              console.error('Screenshot capture failed completely, checking for early screenshot:', retryError)
              // Last resort: use early screenshot if available
              if (earlyScreenshot) {
                screenshotDataUrl = earlyScreenshot
                console.log('Using early screenshot as fallback')
              } else {
                screenshotDataUrl = null
                console.error('No screenshot available at all - this will cause UI issues')
              }
            }
          }
        }
